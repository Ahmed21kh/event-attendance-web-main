...
const AddCustomer = ({ open, onCancel, onOk }) => {
  ...
  const addUserSchema = yup.object().shape({
    customer_name: yup
      .string()
      .matches(
        /^[a-zA-Zا-ي]+(\s[a-zA-Zا-ي]+)*$/,
        "الاسم يجب ان يحتوي علي حروف فقط"
      )
      .required("اسم العميل مطلوب"),
    customer_mobile: yup
      .string()
      .typeError("رقم الهاتف غير صحيح")
          ...
          <div className=" w-full mt-7">
            <Row gutter={[24, 24]}>
              ...
              <Col xs={24} md={12} lg={12}>
                <Button
                  size="large"
                  className=" bg-white hover:!bg-white !text-violet-600 !border-violet-600  w-full"
                  onClick={() => {
                    reset({
                      customer_name: "",
                      customer_mobile: "",
