...
const Customers = () => {
  ...
  const getAllCustomers = () => {
    try {
      setloadingData(true);
      getAllCustomersApi(pageNumber, pageSize, searchQuery)
        .then((res) => {
          console.log(res);
          let newData = res?.data?.map((e, i) => {
            return { key: i, ...e };
          });
      ...
      <div className=" w-full h-full px-5 pt-[80px] overflow-auto ">
        ...
        <form onSubmit={handleSubmit(handleSearch)} className=" my-4">
          <Row dir="rtl" gutter={[24, 24]}>
            ...
            <Col xs={24} md={12} lg={{span:4,offset:6}} xl={{span:4,offset:6}} xxl={{span:3,offset:7}}>
                <Button
                  type="primary"
                  size="large"
                  className="w-full bg-violet-600 flex items-center justify-center hover:!bg-violet-500"
