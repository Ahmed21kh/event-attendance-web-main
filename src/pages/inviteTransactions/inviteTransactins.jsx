...
import { getInvitationsApi, getInviteTransactionApi } from "../../services/Apis";
...
function InviteTransactins() {
  ...
  const { setChecklist, checklist, handleSendMessage, loadingBtn, socket } =
    useContext(Context);
 ...
 const getAllInvitations = async () =>{
  try {
    setloading(true);
    await getInvitationsApi().then((res)=>{
      setinvitations(res)
      setloading(false)
    }).catch((err) => {
  ...
  const getAllinviteTransactions = () => {
    try {
      setloadingData(true);
      getInviteTransactionApi(pageNumber, pageSize, searchQuery)
        .then((res) => {
          console.log(res);
          let newData = res[0]?.data?.map((e, i) => {
            return { key: e._id, ...e };
          });
  ...
  const handleCheckboxChange = (selectedKeys, data) => {
    ...
    setChecklist([
      ...data?.map((e) => {
        return {
          [e._id]: {
            phone: e.customerDetails.customer_mobile,
            msg: `السيد ${e.customerDetails.customer_name} انت مدعو لحضور ${e.inviteDetails.invite_desc}`,
            id: e._id,
          },
        };
      }),
  ...
  const handleSearch = (data) => {
    ...
    if (sending_status && sending_status !== "all") {
  ...
  const sendMessage = (data) => {
    ...
    const whatsappUrl = `https://wa.me/${customer_mobile}?text=${encodeURIComponent(
      message
  ...
  const columns = [
    {
      title: "اسم العميل",
      render: (records) => (
        <span>{records?.customerDetails?.customer_name}</span>
      ),
    },
    {
      title: "رقم الهاتف",
      render: (records) => (
        <span>{records?.customerDetails?.customer_mobile}</span>
      ),
    },
    {
      title: "الدعوة",
    ...
    },
    {
      ...
      render: (records) => (
        <span>{records?.inviteDetails?.to_date?.split("T")[0]}</span>
      ),
    },
    {
      title: "حالة الحضور",
      ...
      render: (record) => (
        <span
          className={`${record == "attend" ? "text-[green]" : "text-[red]"}`}
        >
          {record == "attend" ? "تم الحضور" : "لم يحضر"}
        </span>
      ),
    },
    {
      ...
      render: (record) => (
        <span className={`${record == "sent" ? "text-[green]" : "text-[red]"}`}>
          {record == "sent" ? "تم الارسال" : "لم يرسل اليه"}
        </span>
      ),
    },
    // {
    ...
    // },
    {
      ...
      render: (record) => (
        <span
          onClick={() => openInviteModal(record?.inviteDetails)}
            ...
            <Col xs={24} md={12} lg={4}>
              <Controller
                ...
                render={({ field }) => (
                  ...
                  className="w-full"
                  options={invitations.map((invite) =>{return {label: invite.invite_name , value: invite._id}})}
                />
                )}
              />
            </Col>
              ...
              <Controller
                ...
                render={({ field }) => (
                  <Select
                    dropdownStyle={{ direction: "rtl" }}
                    {...field}
                    placeholder="ابحث بحالة الارسال"
                    size="large"
                    className="w-full"
                    options={[
                      { label: "الكل", value: "all" },
                      { label: "تم الارسال", value: "sent" },
                      { label: "لم يتم الارسال", value: "not_sent" },
                    ]}
                  />
                )}
              />
              ...
              <Row gutter={[24, 24]}>
                <Col xs={24} md={12} lg={12}>
                  <Button
                    ...
                    size="large"
                    htmlType="submit"
                    className=" w-full bg-violet-600 flex items-center justify-center hover:!bg-violet-500"
                ...
                <Col xs={24} md={12} lg={12}>
                  <Button
                    type="primary"
                    size="large"
                    className=" w-full bg-white text-violet-600 border-violet-600 flex items-center justify-center hover:!bg-white hover:!text-violet-600"
      ...
      >
        <div className=" p-5">
          <Input.TextArea
            rows={3}
            className=" w-full"
            placeholder="اكتب الرسالة التي تريد ارسالها...."
          />
        </div>
        <div className=" flex gap-3 justify-end py-3 items-center">
          <Button
            type="primary"
            className=" bg-violet-500 flex items-center hover:!bg-violet-600"
            onClick={handleOk}
          >
            ارسال
          </Button>
        ...
        <Row gutter={[24, 24]} className="py-5">
          ...
          <Col xs={24} lg={12}>
            ...
            <span className=" text-lg">
              {inviteDetails?.from_date?.split("T")[0]}
            </span>
          </Col>
          <Col xs={24} lg={12}>
            <p className=" text-md text-gray-300">تاريخ النهاية</p>
            <span className=" text-lg">
              {inviteDetails?.to_date?.split("T")[0]}
            </span>
          </Col>
        </Row>
        <div className=" flex gap-3 justify-end py-3 items-center">
